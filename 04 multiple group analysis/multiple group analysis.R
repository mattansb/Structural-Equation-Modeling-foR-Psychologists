...
fit_groups <- sem(mod, data = group_data, group = "gender") # tell lavaan what the grouping var is
...
fit_groups <- sem(mod, data = group_data, group = "gender")
summary(fit_groups, standardize = TRUE)
...
fit_groups_eq_cov <- sem(mod_eq_cov, data = group_data, group = "gender")
...
fit_meas <- cfa(meas_mod, data = group_data, group = "gender")
