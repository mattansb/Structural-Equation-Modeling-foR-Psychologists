...
### 2.1. Define each **latent variable** by its indicators (`=~`)

```r
Lat =~ ind1 + ind2 + ind3
```

### 2.2. Build the regression formula for each **endogenous variable** according to its *incoming* arrows (`~`)
...
### 2.3. Define the co/variances for each **two headed arrow** (`~~`)

These include auto-correlations,  
and possibly any error / disturbances / uniqueness variances.
...
### 2.4. Define any computed parameters (paths, differences...) (`:=`)
